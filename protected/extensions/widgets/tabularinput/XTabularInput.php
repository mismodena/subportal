		...
		$.ajax({
			success: function(html){                                
				...
				input.siblings('.{$this->headerCssClass}').show();
                                ...
                                jQuery('#FppDetail_'+index+'_fppInvNo').select2({'placeholder':'-- pilih --'});                                
                                jQuery('#ProformaDetail_'+index+'_itemModel').select2({'placeholder':'-- item --'});      
                                jQuery('#MutationDetail_'+index+'_assetID').select2({'placeholder':'-- asset --'});  
                                jQuery('#DisposalDetail_'+index+'_assetID').select2({'placeholder':'-- asset --'});     
                                jQuery('#DocumentReqDetail_'+index+'_docNumber').select2({'placeholder':'-- faktur --'}); 
                                jQuery('#DocumentReqDetail_'+index+'_retType').select2({'placeholder':'-- Status --'}); 
                                jQuery('#DocumentTTTDetail_'+index+'_docNumber').select2({'placeholder':'-- faktur --'}); 
                                jQuery('#DocumentTTTDetail_'+index+'_retType').select2({'placeholder':'-- Status --'});
                                jQuery('#BQItem_'+index+'_itemNo').select2({'placeholder':'-- Status --'});
...
SCRIPT;
	...
	protected function renderContent()
	{
            foreach($this->models as $index=>$model)
		{
			...
			$this->controller->renderPartial($this->inputView, array('model'=>$model, 'index'=>$index, 'param' => $this->params));
			if($this->isRemoveOk){
	...
	{
		$imghtml = CHtml::image(Yii::app()->request->baseUrl.'/css/skins/gridview/delete.png','Remove');
		$removeLink=CHtml::link($imghtml, '#', $this->removeHtmlOptions).'<input type="hidden" class="'.$this->indexCssClass.'" value="'.$index.'" />';
