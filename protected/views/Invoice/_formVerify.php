        ...
        <div>
            <table border="0" width="50%">
                <tr>
                    <td>
                        <div class="simple">                            
                            <?php echo $form->hiddenField($model, 'invID', array('size' => 40, 'maxlength' => 50,)); ?>                            
                            <?php echo $form->hiddenField($model, 'status', array('size' => 40, 'maxlength' => 50,)); ?>                            
                        </div>                               
                    </td>                        
                </tr>            
                <tr>
...
function verified()
{    
    $("#ProformaInvoice_status").val("verified");
    var data=$("#inv-header-form").serialize();
    //alert(data);
    //document.forms[0].submit();
    $.ajax({
        type: 'POST',
        url: '<?php echo Yii::app()->createUrl('invoice/approval'); ?>',
        data:data,
        success:function(data){
            var message = 'Proforma Invoice Nomor '+data.invNo+' '+' telah diverifikasi';
            var redirect = '<?php echo Yii::app()->createUrl("invoice/verifyPI"); ?>'
            alert(message); 
