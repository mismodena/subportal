<div class="form">
    ...
    Yii::app()->clientScript->registerScript('JQuery', "
        ...
        {
            var sum = 0;
            $('#tablePI > tbody  > tr').each(function() {
                var qty = $(this).find('.qty').val();
                var model = $(this).find('.model').val();
                var price = $(this).find('.price').val();                
                
                if(model !== '')
                {var amount = (qty*price);} else {var amount = 0;}
	...
	<?php echo $form->errorSummary($model); ?>
        ...
        <?php 
            $this->widget('ext.widgets.tabularinput.XTabularInput', array(
                ...
                'inputTagName' => 'tr',
                'inputView' => '/site/extensions/_tabularPI',
                'inputUrl' => $this->createUrl('request/addTabProforma'),
                'addTemplate' => '<tbody><tr></td><td colspan="5">{link}</td></tr></tbody>',
